...
class Postcode{
	...
	public function lookup($postcode){
		$jsonurl = "http://api.postcodes.io/postcodes/".$postcode;
	...
	public static function validate($postcode){
		$jsonurl 	= "http://api.postcodes.io/postcodes/".$postcode."/validate";
		//$json 	= $this->request($jsonurl);
		$pcode 		= new Postcode;
		$json 		= $pcode->request($jsonurl);
		$decoded 	= json_decode($json);
		if($decoded->status == 200){
			if($decoded->result == 1){
				return true;
	...
	public static function distance($postcode1, $postcode2, $unit){
	...
	//	$postcode2 = $this->lookup($postcode2);
		$pcode 	=	new Postcode;
		$postcode1 = $pcode->lookup($postcode1);
		$postcode2 = $pcode->lookup($postcode2);
		...
		else if ($unit == "N") { return ($miles * 0.8684); } else {
		    return $miles;
		}	
	}
	public function request($jsonurl){
		...
		curl_setopt($ch, CURLOPT_CUSTOMREQUEST,'GET');curl_setopt($ch, CURLOPT_HTTPHEADER,['Content-Type:application/json',]);
...
add_action('plugins_loaded',array('Postcode','init'));
?>